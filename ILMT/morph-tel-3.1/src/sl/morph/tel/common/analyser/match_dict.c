...
int match_in_dict(guessed_root,pdgm,cat,offset,ans_table,ans_ar,loop3)
  ...
  struct ans_struct ans_ar[Arraysize];    /* is structure of root word,auxilary word ,paradigm
                                             and offset value */
   {
	...
	//if(!strcmp(ans_table[loop2].pdgm,pdgm)&&!strcmp(ans_table[loop2].cat,"adj"))
	strcpy(cat,ans_table[loop2].cat);
	//printf("cat=%s,dict_cat=%s\n",cat,ans_table[loop2].cat);
...
//blocked NAgaraj and Paramesh to stop printing caratM caratN chars in root.
////ans_ar[*loop3].root[0] = ans_table[loop2].priority;
		...
		//  strcat(ans_ar[*loop3].root,"_");
		  ...
		  strcpy(ans_ar[*loop3].cat,ans_table[loop2].cat);
		  if(!INFLTNL)  /* if flag is true */
