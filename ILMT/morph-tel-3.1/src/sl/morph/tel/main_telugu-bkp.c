...
//sprintf(logmsg, "INFO: XXXXXXXXXXX=%s",fname); PRINT_LOG(log_file, logmsg);
	...
	sprintf(logmsg, "INFO: fun_morph() returned |%s|%d|%s|cat=%s|", ans_ar[loop1].root, ans_ar[var].offset, ans_ar[var].pdgm,category); PRINT_LOG(log_file, logmsg);
		//printf("rcat=%s|cat=%s|\n",ans_ar[loop1].cat,cat);
...
//printf ("PARAMESH--lcat_variation=|%s|\n",lcat_variation[0]);
	...
	strcpy(lcat_variation[0],"nst");
		...
		//printf("cat=%s\n",ans_ar[loop1].cat);
		if(ans_ar[0].offset!=-2){
		strcpy(category,ans_ar[0].cat);
		memset(ans_ar[loop1].cat,'\0',sizeof(ans_ar[loop1].cat));}
	...
	//printf("INFO: Calling |morph=%s|lcat_veriation=%s|root_category=|%s\n",morph, cat, category); 
		if(category[0]=='\0') {
		strcat(feature_str, cat);
		//printf("root=%s|cat=%s\n",ans_ar,cat);
		}
