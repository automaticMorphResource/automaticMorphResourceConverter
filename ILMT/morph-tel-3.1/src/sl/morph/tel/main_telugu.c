	...
	char *argv[];
{
	...
	char left_cat[BigArray];
	char right_cat[BigArray];
...
//sprintf(logmsg, "INFO: XXXXXXXXXXX=%s",fname); PRINT_LOG(log_file, logmsg);
	...
	//fun_morph(morph,fp_pdgm,DBM_FLAG,db_dict,db_suff,ans_ar,&no_of_ans);//to add category
	fun_morph(morph,category,fp_pdgm,DBM_FLAG,db_dict,db_suff,ans_ar,&no_of_ans);
	
	sprintf(logmsg, "INFO: fun_morph() returned |%s|off1=%d|off2=%d|off3=%d|%s|cat=%s|", ans_ar[loop1].root, ans_ar[0].offset,ans_ar[1].offset,ans_ar[2].offset, ans_ar[var].pdgm,category); PRINT_LOG(log_file, logmsg);
	//printf("ans_ar0=%s|ans_ar1=|%s|cat=|%s|\n",ans_ar[0].cat,ans_ar[1].cat,category);
	if(!strcmp(category,"pro")){
	strcpy(category,"pn");
	}
...
//  fpath  -- /home/teltamilmt/sampark/data_bin/sl/morph/tel/
	...
	if (!ispunct(c))
	{
		...
		int ret_status=0;
		category[0]='\0'; // derived word category null
		//memset(category,'\0',sizeof(category));
		ret_status = sandhi_split (morph, left_cat, right_cat, &rule_pool, fp_pdgm, DBM_FLAG, db_dict,db_suff, lft_word, rt_word,ans_ar_left,ans_ar_right,&no_of_ans_left,&no_of_ans_right);
		sprintf(logmsg, "INFO: left word and right word after sandhi split,now in main |%s|%s|%s|%s|l_cat=%s|r_cat=%s|",lft_word,rt_word,ans_ar_left[0].root,ans_ar_right[0].root,left_cat, right_cat); PRINT_LOG(log_file, logmsg);
	strcpy(left_cat,ans_ar_left[0].cat);
	strcpy(right_cat,ans_ar_right[0].cat);
	//printf("lcat=%s|rcat=%s\n",left_cat,right_cat);
	//printf("category=%s\n",left_cat);
...
//	ans_ar_tmp = ans_ar;
	...
	//memset(category,'\0',sizeof(category));
	//printf("cat=%s\n",left_cat);
	/*if(strcmp(cat,left_cat))	//cat=derived/offset category; category=root/dict category
	strcpy(category,left_cat); 
	memset(left_cat, '\0', sizeof(left_cat));*/
	}
	else if( flag_unk == 1 && word_count == 1 ){
	//	printf("the ans_ar_right %s\n", ans_ar_right);
	ans_ar_tmp = ans_ar_right;
	loopsize = no_of_ans_right;
	//strcpy(cat,right_cat);
	//memset(category, '\0', sizeof(category));
	/*if(strcmp(cat,right_cat))
	strcpy(category,right_cat); 
	memset(right_cat, '\0', sizeof(right_cat));*/
...
// paramesh: NCR Values are introduced here 310112 ...Ending
	...
	//if(ans_ar[0].offset!=-2)
	strcpy(category,ans_ar_tmp[loop1].cat);
	if(!strcmp(category,"pro"))
	strcpy(category,"pn");
	...
	//memset(category,'\0',sizeof(category));
	//printf("root=%s|category=%s|cat=%s|\n",ans_ar_tmp[loop1].root,ans_ar_tmp[loop1].cat,cat);
	//}
	//printf("root=%s|ans_category=%s|cat=%s|\n",ans_ar_tmp[loop1].root,ans_ar_tmp[loop1].cat,cat);
	...
	sprintf(logmsg, "INFO: Calling resufun_hori(),root=%s|cat=%s|g=%s|n=%s|p=%s|case=%s|cm=%s|suff=%s|", root, cat, g, n, p, kase, cm, tam ); PRINT_LOG(log_file, logmsg);
	//printf("ans_ar[1].pdgm=%s\n",ans_ar[1].pdgm);
	if(((!strcmp(ans_ar[0].cat,"n") && !strcmp(ans_ar[1].cat,"v")) || (!strcmp(ans_ar[0].cat,"v") && !strcmp(ans_ar[1].cat,"n"))) && !strcmp(cat,"v"))
	strcpy(category,"v");
	if(((!strcmp(ans_ar[0].cat,"n") && !strcmp(ans_ar[1].cat,"v")) || (!strcmp(ans_ar[0].cat,"v") && !strcmp(ans_ar[1].cat,"n"))) && !strcmp(cat,"n"))
	strcpy(category,"n");
	
	
	//if(!strcmp(cat,"v"))
	//strcpy(category,"v");
	//printf("root=%s|category=%s|category=%s|cat=%s|\n",ans_ar_tmp[loop1].root,ans_ar_tmp[loop1].cat,category,cat);
	//printf("root=%s|category=%s|cat=%s|\n",ans_ar_tmp[loop1].root,category,cat);
//
...
//printf ("PARAMESH--lcat_variation=|%s|\n",lcat_variation[0]);
	...
	//strcat(feature_str, category);
	sprintf(logmsg, "INFO: Calling |lcat_veriation=%s|root_category=|%s", cat, category); PRINT_LOG(log_file, logmsg);
		if(category[0]=='\0') {
		strcat(feature_str, cat);
		//printf("root=%s|cat=%s\n",ans_ar,cat);
		}
...
//	if(!strcmp(cm,"0_avy")||!strcmp(cm,"0_adj"))
//	strcpy(cm,"0");
//	strcat(feature_str, cm);
//memset(cm,'\0',sizeof(cm));
//	strcat(feature_str, ",");
// To delete cat in seventh field
	char suff_cm[5];
        memset(suff_cm,'\0',strlen(suff_cm));
        strcpy(suff_cm,cm+strlen(cm)-4);
        int l=strlen(cm)-4;
        char cm1[10];//Added to remove _adj _adv _avy in seventh field 10-09-2013 -Nagaraju
        memset(cm1,'\0',strlen(cm1));
        //if(!strcmp(cm,"0_avy")||!strcmp(cm,"0_adj"))
        if(!strcmp(suff_cm,"_avy")||!strcmp(suff_cm,"_adj")||!strcmp(suff_cm,"_adv")){
        strncpy(cm1,cm,l);
        strcpy(cm,cm1);
