...
//int sandhi_split (char *input_word, struct sandhi_rule_pool *rule_pool,
int sandhi_split (char *input_word, char *left_cat, char *right_cat, struct sandhi_rule_pool *rule_pool,
		FILE * fp_pdgm, int DBM_FLAG, GDBM_FILE db_lex,
		...
		int* no_of_ans_left, int* no_of_ans_right,int *split_flag)
{
	...
	{				// for every rule in rule pool
		...
		if (pattern_matched[i] == 1) {
			...
			fun_morph (test_left, left_cat, fp_pdgm, DBM_FLAG, db_lex, db_suff, ans_ar_left,no_of_ans_left);
...
// commented to solve 64-bit problem
				//if((strcmp(cat, rule_pool->rule[i].lft_cond)==0 || strcmp(catl,rulecatl)==0 || strcmp(catr,rulecatr)==0)&& (strcmp(cat,",")==1))   {
				if((strcmp(cat, rule_pool->rule[i].lft_cond)==0 || strcmp(catl,rulecatl)==0 || strcmp(catr,rulecatr)==0)&& (strcmp(cat,",")!=0))   {
				...
				fun_morph (test_right, right_cat, fp_pdgm, DBM_FLAG, db_lex, db_suff, ans_ar_right,no_of_ans_right);
