...
//										printf("answer going ot print\n");
	...
	resufun_hori(ans_ar_tmp[loop1].root,ans_ar[loop1].pdgm,ans_ar[loop1].offset,ans_ar[loop1].aux_verb,root, cat, g,n,p, kase,cm, tam );
	sprintf(logmsg, "INFO: Calling resufun_hori(),|root=%s|cat=%s|g=%s|n=%s|p=%s|case=%s|cm=%s|suff=%s|", root, cat, g, n, p, kase, cm, tam ); PRINT_LOG(log_file, logmsg);
//
	 resufun_hori(ans_ar_tmp[loop1].root,ans_ar[loop1].pdgm,ans_ar[loop1].offset,ans_ar[loop1].aux_verb,root, cat, g,n,p, kase,cm, tam     );
	 sprintf(logmsg, "INFO: Calling resufun_hori(),|root=%s|cat=%s|g=%s|n=%s|p=%s|case=%s|cm=%s|suff=%s|", root, cat, g, n, p, kase    , cm, tam ); PRINT_LOG(log_file, logmsg);

//													                                                                                                        resufun_hori(ans_ar_tmp_right[loop1].root,ans_ar_right[loop1].pdgm,ans_ar_right[loop1].offset,ans_ar_right[loop1].aux_verb,root, cat, g,n,p, kase,cm, tam );
