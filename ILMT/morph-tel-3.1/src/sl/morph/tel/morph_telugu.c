...
void fun_morph(morph,category,fp_pdgm,DBM_FLAG,db_lex,db_suff,ans_ar,no_of_ans)
	...
	//char category[5][10];
{
	sprintf(logmsg, "INFO: fun_morph"); PRINT_LOG(log_file,logmsg);
	...
	//char pdgm[WORD_SIZE],category[CATEGORYWORDSIZE],my_pdgm[WORD_SIZE];//category is declared in function
	...
	sprintf(logmsg, "INFO: Entering in AVY_srch() with word:|%s|",morph); PRINT_LOG(log_file,logmsg);
	...
	if (ISNUM !=  -3) {	
		...
		for(loop=0;loop<2 && !pos;loop++) {
			...
			for(loop3= morph_size;loop3>=0;loop3--)/* executes until loop3 value becomes 0 */
			{
				...
				sprintf(logmsg, "INFO: Assumed suffix from get_suff() for word|%s| suff|%s| ", morph, suff); \
				...
				if(DBM_FLAG)	/* executes when the word exists */
				{
					...
					sprintf(logmsg, "INFO: Assumed tag suffix from dbm_suff_tab_lookup() for tag_suff|%s| ", tag_suff); \
						PRINT_LOG(log_file,logmsg);
				...
				sprintf(logmsg, "INFO: Assumed tag suffix from my_blk_bsearch() for tag_suff|%s| ", tag_suff); \
					PRINT_LOG(log_file,logmsg);
				loop1=0;
				while(tag_suff!=NULL)	/* executes until tag_suff!=null */
				{
					...
					sprintf(logmsg, "INFO: After deleting from root the suffix to be added from cp_suff_add_struct() for tag_suff|%s| ", tag_suff); 
					PRINT_LOG(log_file,logmsg);
				...
				if(loop1>0)	/* checks for loop1>0 */
				{
					...
					{
						sprintf(logmsg, "INFO:while loop of guessed_rt() |guessed_root=%s|morph=%s|",guessed_root,morph );
						PRINT_LOG(log_file,logmsg);
				...
				//printf("cat=%s\n",ans_table[loop2].cat);
						/*if(strlen(ans_table[loop2].cat)!=0&&strlen(ans_ar1)!=0){
						strcpy(category[loopc],ans_table[loop2].cat);
						printf("category[loop%d]=|%s|\n",loopc,category[loop2]);
						loopc++;
						}*/
						sprintf(logmsg, "INFO:guessed_root after dict_lookup() |guessed_root=%s| ans_ar1|%s|", 
								guessed_root, ans_ar1); \
							PRINT_LOG(log_file,logmsg);
						...
						for(loop1=0;loop1<tag_suff_ar[loop2].no_of_entries ; loop1++)
						{
							...
							//sprintf(logmsg, "INFO: get PDGM info from cat |PDGM=%s|CAT=%s|", pdgm, category); \
							PRINT_LOG(log_file,logmsg);
							...
							sprintf(logmsg, "INFO:verify_root() |guessed_root=%s|category %s|", 
									guessed_root, category); \
								PRINT_LOG(log_file,logmsg);
					if(!INFLTNL) {
						...
						sprintf(logmsg,"dict_category=%s\n",category);PRINT_LOG(log_file,logmsg);
						}
						/* this function checks the word in the  dictionary */	
		...
		*no_of_ans = loop;
		...
		if (!FINDONCE) 	 /* checks for FINDONCE is false */
		{
			...
			sprintf(logmsg, "INFO: The default ans is |morph=%s|", morph);
			PRINT_LOG(log_file,logmsg);
		}
	}
	strcpy(ans_ar[pos].root,morph);
