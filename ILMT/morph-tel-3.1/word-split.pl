...
### 231111
	...
	$word=~s/nagaramve\t/nagarame\t/;#sAmrAjyaMlonu
	$word=~s/ceyagalg/ceyagalug/;#ceyagalginavAru
	$word=~s/cUpabadiMxi/cUdabadiMxi/;#ceyagalginavAru
##added by tel-hin.word-split.pl-28feb2014-nvs
...
##if($word=~/([A-z]+)n([ui])M([cd])([IUAeo])\t/)        ## elYlanuMci 17112012gurao
	...
	if($word=~/([A-z])nenA\t/) { ##awanenA gurao27082012
	$word=~/(.*)nenA(.*)/;
	$word=$1."nu\tunk";
	$word1="enA".$2;
	}
...
#	if($word=~/[dr]EnA|[dr]Ewe/) { # eVvarEnA, eVvarEwe, puMdEwe ##blocked-telhin
#	$word=~/(.*)E(.*)/;
#	$word=$1."u\tunk";
#	$word1="E".$2;
#	}
...
#	if($word=~/inaMxu/) { # gadipinaMxulake ##blocked telhin
#	$word=~/(.*)ina(.*)/;
#	$word=$1."ina\tunk";
#	$word1="a".$2;
#	}
...
#080610
	...
	if($word=~/([A-z])evani\t/) { #vaccevani 210111
	$word=~/(.*)vani\t(.*)/;
	$word=$1."vi\tunk";
	$word1="ani\t".$2;
	}
...
#	if($word=~/[oa]leni\t/) {  	## maruvaleni, werukoleni, 210211-BLOCKED becoz double splitting is needed here UhiMcalenaMwa-blocked-telhin 
#	$word=~/(.*)leni\t(.*)/;
#	$word=$1."\tunk";
#	$word1="leni\t".$2;
#	}
...
#}
	...
	if($word=~/([A-z])ayyAvaMta/) {  	##  ayyAvaMta
	$word=~/(.*)ayyAvaMta(.*)/;
	$word=$1."ayyAvu\tunk";
	$word1="aMta".$2;
	}
...
#Temperarily splitting it, becoz the first analysis has reduplication
	...
	if($word=~/([A-z])nnappud/) {  	##walacukunnappudallA
	$word=~/(.*)nnappud(.*)/;
	$word=$1."nna\tunk";
	$word1="appud".$2;
	}
...
#	}
	...
	if($word=~/nenadigAnannAdu/) {  	#nenadigAnannAdu
	$word=~/(.*)nenadigAnannAdu(.*)/;
	$word=$1."nenu\tunk";
	$word1="adigAnu".$2;
	$word2="annAdu".$2;
	}
	if($word=~/wuMxana/) {  	#sAXyapaduwuMxanataMlo
	$word=~/(.*)wuMxana(.*)/;
	$word=$1."wuMxi\tunk";
	$word1="ana".$2;
	}
	if($word=~/lugAvidipoyiMxi/) {  	#sAXyapaduwuMxanataMlo
	$word=~/(.*)lugAvidipoyiMxi(.*)/;
	$word=$1."gA\tunk";
	$word1="vidipoyiMxi".$2;
	}
#sAXyapaduwuMxanataMlo
####################################################
