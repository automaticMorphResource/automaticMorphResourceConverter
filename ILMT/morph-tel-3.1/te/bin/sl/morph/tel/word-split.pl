#### THIS IS SIMPLE SANDHI SPLITTER FOR TELUGU --TEL-TAM MT######
#Oct, 2008
#PARAMESHKRISHNA
...
### 231111
	...
	$word=~s/piluvabad/pilavabad/;#badu forms are chopped; in that case piluva to be pilava to analyse correctly
...
#	if($word=~/([A-z]+)n(unna)$/) {  #ixamixxaMgAceVppalemaMtunnAru
#        $word=~/([A-z]+)n(unna)$/;
#	$word=$1."n\tunk";
#	$word1=$2;
#}
...
#	}
	...
	if($word=~/^([0-9]).*([0-9])([A-z])/) { # 600maMxi, 10kotla
	$word=~/(.*)([0-9])([A-z])(.*)/;
	$word=$1."$2\tunk";
	$word1=$3.$4;
	}
...
##	$word1="mAwraM"."\tunk";
	...
	if($word=~/nappudu\t/) {	##bawukuwunnappudu
	$word=~/(.*)nappudu(.*)/;
	$word=$1."na\tunk";
	$word1="appudu".$2;
	}
...
#
	...
	if($word=~/nappatik/) { # kriwaMnAtivani
	$word=~/(.*)nappatik(.*)/;
	$word=$1."na\tunk";
	$word1="appatik".$2;
	}
	...
	if($word=~/wunnaxani\t|exani\t|inaxani\t/) { # waleVwwuwunnaxani  [nirNayiMcabaduwuMxani] ^lexani, *jarugaxani
	$word=~/(.*)xani(.*)/;
	$word=$1."xi\tunk";
	$word1="ani".$2;
	}
...
#050610
	...
	if($word=~/llAge\t/) { ##xaMpawullAge
	$word=~/(.*)llAge(.*)/;
	$word=$1."la\tunk";
	$word1="lAge".$2;
	}
	if($word=~/maMwati/) { ##xeSamaMwatilo,n         M+aMw
	$word=~/(.*)maMwati(.*)/;
	$word=$1."M\tunk";
	$word1="aMwati".$2;
	}
	if($word=~/naMwati/) {##adavinaMwatinI
	$word=~/(.*)naMwati(.*)/;
	$word=$1."ni\tunk";
	$word1="aMwati".$2;
	}
	...
	if($word=~/llAMtivi\t/) {##portullAMtivi
	$word=~/(.*)llAMtivi\t(.*)/;
	$word=$1."lu\tunk";
	$word1="lAMtivi\t".$2;
	}
	if($word=~/ulOu\t/) {##vimukwulOu
	$word=~/(.*)lOu(.*)/;
	$word=$1."lu\tunk";
	$word1="avvu".$2;
	}
...
#xeSamaMwatilo,n         M+aMwati
	if($word=~/EunnA([dnmvry])u\t/) { # kriwaMnAtivani
	$word=~/(.*)EunnA(.*)/;
	$word=$1."E\tunk";
	$word1="unnA".$2;
	}
...
### becoz aMtunnAdu => *aMtu+unnAdu
	...
	#$word1="unnA".$2;
	#}
	if($word=~/uMxaMtunnA[dnmr]u\t/) { # kaluguwuMxaMtunnAru
	$word=~/(.*)aMtunn(.*)/;
	$word=$1."i\tunk";
	$word1="aMtunn".$2;
	}
...
#xeSamaMwatilo,n         M+aMwati
	...
	if($word=~/[A-z][^an][n]aMwa\t/) { # UhiMcalenaMwa, *wanaMwa	unk* winnaMwa
	$word=~/(.*)naMwa(.*)/;
	$word=$1."ni\tunk";
	$word1="aMwa".$2;
	}
...
#070610
#
	if($word=~/amicc/) { # aMkiwamiccAdu, samAXAnamiccinatlu
	$word=~/(.*)amicc(.*)/;
	$word=$1."aM\tunk";
	$word1="icc".$2;
	}
	if($word=~/MxaMte\t/) { #"axemitaMte, vacciMxaMte
	$word=~/(.*)MxaMte(.*)/;
	$word=$1."Mxi\tunk";
	$word1="aMte".$2;
	}
	if($word=~/kkuvane\t/) { #wakkuvane
	$word=~/(.*)ane(.*)/;
	$word=$1."a\tunk";
	$word1="ane".$2;
	}
#	if($word=~/[mvr]ane\t/) { #vacciMxane, vyakwiwvamanexi, *uMdane, *
	...
	if($word=~/amaMwatA\t/) { #rAjulanodiMcAdu
	$word=~/(.*)amaMwatA(.*)/;
	$word=$1."aM\tunk";
	$word1="aMwatA".$2;
	}
	...
	if($word=~/aMnuMdI\t/) { #rAjulanodiMcAdu
	$word=~/(.*)aMnuMdI(.*)/;
	$word=$1."aM\tunk";
	$word1="nuMdI".$2;
	}
...
#080610
	...
	if($word=~/evani\t/) { #vaccevani 210111
	$word=~/(.*)vani\t(.*)/;
	$word=$1."vi\tunk";
	$word1="ani\t".$2;
	}
...
#	}
	...
	if($word=~/lalonu\t/) {  	## 3       aMculalonu      unk     <fs af='aMculalonu,unk,,,,,,'>
	$word=~s/lalonu/laloni/;
	}
	if($word=~/^muKaputaM/) {  	## muKaputaM
	$word=~s/muKaputaM/muKaputa/;
	}
	if($word=~/లకు\t/) {  	## 500లకు
	$word=~s/లకు/laku/;
	}
	if($word=~/isArilAne\t/) {  	## prawisArilAne
	$word=~/(.*)isArilAne(.*)/;
	$word=$1."i\tunk";
	$word1="sArilAne".$2;
	}
	if($word=~/udoVkaru\t/) {  	## nAvikudoVkaru
	$word=~/(.*)udoVkaru(.*)/;
	$word=$1."udu\tunk";
	$word1="oVkaru".$2;
	}
...
#	}
	...
	if($word=~/wonna\t/) {  	## AkarRiswonna
	$word=~/(.*)wonna(.*)/;
	$word=$1."wU\tunk";
	$word1="unna".$2;
	}
	if($word=~/nnenu\t/) {  	## annAnnenu
	$word=~/(.*)nnenu(.*)/;
	$word=$1."nu\tunk";
	$word1="nenu".$2;
	}
	if($word=~/Asare\t/) {  	## unnAsare
	$word=~/(.*)Asare(.*)/;
	$word=$1."A\tunk";
	$word1="sare".$2;
	}
	if($word=~/xavadAni/) {  	## woVmmixavadAniki
	$word=~/(.*)xavadAni(.*)/;
	$word=$1."xi\tunk";
	$word1="avadAni".$2;
	}
	if($word=~/AnaMwaraM\t/) {  	## add in pc_data:: yAwrAnaMwaraM
	$word=~/(.*)AnaMwaraM(.*)/;
	$word=$1."a\tunk";
	$word1="waruvAwa".$2;
	}
	if($word=~/amayyiMxi\t/) {  	## rAjyamayyiMxi
	$word=~/(.*)amayyiMxi(.*)/;
	$word=$1."aM\tunk";
	$word1="ayyiMxi".$2;
	}
	if($word=~/aMkaMte\t/) {  	## saMpraxAyaMkaMte
	$word=~/(.*)aMkaMte(.*)/;
	$word=$1."aM\tunk";
	$word1="kaMte".$2;
	}
	if($word=~/AraMte\t/) {  	## waralivaswuMtAraMte
	$word=~/(.*)AraMte(.*)/;
	$word=$1."Aru\tunk";
	$word1="aMte".$2;
	}
...
#}
	...
	if($word=~/AlsirAva/) {  	##  caxavAlsirAvataMwo
	$word=~/(.*)AlsirAva(.*)/;
	$word=$1."Alsi\tunk";
	$word1="rAva".$2;
	}
	...
	if($word=~/kolenann/) {  	## waleVwwukolenannAdu
	$word=~/(.*)lenann(.*)/;
	$word=$1."lenu\tunk";
	$word1="ann".$2;
	}
	if($word=~/vukaxA\t/) {  	## annAvukaxA
	$word=~/(.*)vukaxA(.*)/;
	$word=$1."vu\tunk";
	$word1="kaxA".$2;
	}
	...
	if($word=~/laMxaraM\t/) {  	## snehiwulaMxaraM
	$word=~/(.*)laMxaraM(.*)/;
	$word=$1."la\tunk";
	$word1="aMxaraM".$2;
	}
	if($word=~/attanipiM/) {  	## 'rAsinattanipiMcixirA
	$word=~/(.*)attanipiM(.*)/;
	$word=$1."attu\tunk";
	$word1="anipiM".$2;
	}
	...
	if($word=~/naMwe\t/) {  	## caxivAnaMwe
	$word=~/(.*)naMwe(.*)/;
	$word=$1."nu\tunk";
	$word1="aMwe".$2;
	}
	if($word=~/xayyiM/) {  	## eVnimixayyiMxi
	$word=~/(.*)xayyiM(.*)/;
	$word=$1."xi\tunk";
	$word1="ayyiM".$2;
	}
	...
	if($word=~/raMtAr[Aeou]\t/) {  	## rAsAraMtAru , eVvaraMtArA
	$word=~/(.*)raMtAr(.*)/;
	$word=$1."ru\tunk";
	$word1="aMtAr".$2;
	}
	if($word=~/sepayyAka\t/) {  	## kAsepayyAka
	$word=~/(.*)sepayyA(.*)/;
	$word=$1."sepu\tunk";
	$word1="ayyA".$2;
	}
...
#	$word1="povadaM".$2;
#	}
	if($word=~/nattunnA/) {  	## ceswunnattunnAru
	$word=~/(.*)nattunnA(.*)/;
	$word=$1."nattu\tunk";
	$word1="unnA".$2;
	}
...
#	$word1="galugu".$2;
#	}
	if($word=~/vAlsivuM/) {  	## wIsukurAvAlsivuMtuMxi
	$word=~/(.*)vAlsivuM(.*)/;
	$word=$1."valasi\tunk";
	$word1="uM".$2;
	}
	...
	if($word=~/rAlaMxiMc/) {  	## sahakArAlaMxiMcina
	$word=~/(.*)rAlaMxiMc(.*)/;
	$word=$1."rAlu\tunk";
	$word1="aMxiMc".$2;
	}
...
#	}
	...
	if($word=~/exilexu\t/) {  	## iccexilexu
	$word=~/(.*)exilexu(.*)/;
	$word=$1."exi\tunk";
	$word1="lexu".$2;
	}
	if($word=~/ivalle\t/) {  	## nItivalle
	$word=~/(.*)ivalle(.*)/;
	$word=$1."i\tunk";
	$word1="valle".$2;
	}
...
#	
	...
	if($word=~/EnaveVMtane/) {  	##	pUrwEnaveVMtane
	$word=~/(.*)EnaveVMtane(.*)/;
	$word=$1."Ena\tunk";
	$word1="veVMtane".$2;
	}
...
#	$word1="po".$2;
#	}
	if($word=~/ukeVlYiwe/) {  	##	muMxukeVlYiwe
	$word=~/(.*)ukeVlYiwe(.*)/;
	$word=$1."uku\tunk";
	$word1="veVlYwe".$2;
	}
	if($word=~/nattayi\t/) {  	##	kamminattayi
	$word=~/(.*)nattayi(.*)/;
	$word=$1."nattu\tunk";
	$word1="ayyi".$2;
	}
...
#23	veVligipowunnatlugA	unk	<fs af='veVlugu,v,any,any,any,,i_po_wU_uMdu_ina_atli_po_wU_uMdu_ina_atlugA,i_po_wU_uMdu_ina_atlugA'>|<fs af='veVlugu,v,any,any,any,,i_po_wunna_atlugA,i_po_wunna_atlugA'>
#
#Temperarily splitting it, becoz the first analysis has reduplication
	if($word=~/ipowunnatl/) {  	##vItananniMtinI
	$word=~/(.*)ipowunnatl(.*)/;
	$word=$1."i\tunk";
	$word1="powunnatl".$2;
	}
	if($word=~/nnappud/) {  	##walacukunnappudallA
	$word=~/(.*)nnappud(.*)/;
	$word=$1."nna\tunk";
	$word1="appud".$2;
	}
...
#	$word1="xagina".$2;
#	}
	if($word=~/atamegAka\t/) {  	##uMdatamegAka
	$word=~/(.*)atamegAka(.*)/;
	$word=$1."atame\tunk";
	$word1="gAka".$2;
	}
	if($word=~/vArani\t/) {  	##weVlivigalavArani
	$word=~/(.*)vArani(.*)/;
	$word=$1."vAru\tunk";
	$word1="ani".$2;
	}
	if($word=~/wuMxI\t/) {  	##aMxiswuMxI
	$word=~/(.*)wuMxI(.*)/;
	$word=$1."wuMxi\tunk";
	$word1="I".$2;
	}
...
#	$word1="leka".$2;
#	}
	if($word=~/layiwe\t/) {  	##3	prajalayiwe	unk
	$word=~/(.*)layiwe(.*)/;
	$word=$1."lu\tunk";
	$word1="ayiwe".$2;
	}
	...
	if($word=~/AligAnI\t/) {  	#ivvAligAnI
	$word=~/(.*)AligAnI(.*)/;
	$word=$1."Ali\tunk";
	$word1="gAnI".$2;
	}
	if($word=~/eMtanexi\t/) {  	#eMtanexi
	$word=~/(.*)eMtanexi(.*)/;
	$word=$1."eMti\tunk";
	$word1="anexi".$2;
	}
	...
	if($word=~/A[rdmvn]anI\t/) {  	#nirmiMcAranI
	$word=~/(.*)anI(.*)/;
	$word=$1."u\tunk";
	$word1="anI".$2;
	}
	if($word=~/ananusari/) {  	#godananusariswU
	$word=~/(.*)ananusari(.*)/;
	$word=$1."anu\tunk";
	$word1="anusari".$2;
	}
