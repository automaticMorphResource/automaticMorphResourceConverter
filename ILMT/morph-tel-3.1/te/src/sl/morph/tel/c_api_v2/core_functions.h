			...
			case '|':
				...
				if ( str1[0]!='\0' && str2[0]!='\0')
				{
					...
					else
					{
						...
						//char attr[9][20] ;
						//array values are changed by paramesh_280612 hcu to tackle the below output where tam field contains part of suff too.
//51      cuttabadiyunnaxi        <fs af='cuttu,pn,,sg,,,a_badu_i_uMdu_ina_axa_badu_i_uMdu_ina_axi_0,a_badu_i_uMdu_ina_axi_0'>
						char attr[10][26] ;
...
//	 char *node_fs1=  make_or_node_to_string(tree->OR1);
...
void print_node_to_file(node *tree ,char ind[],FILE *f)
{
	...
	char *node_fs1=  make_or_node_to_string(tree->OR1);
//	printf("\n node_fs %s :",node_fs);
	if(strcmp(tree->rgt5,"") && strcmp(tree->left5,""))
	fprintf(f, "%s\t%s\t%s\t%s\n%s\t%s\t%s\t%s\n",ind, tree->left5, tree->node_tag, node_fs,ind,tree->rgt5,tree->node_tag,node_fs1);
	else if(strcmp(node_fs,""))
	fprintf(f, "%s\t%s\t%s\t%s\n",ind, tree->node_tkn, tree->node_tag,node_fs);	
	else //if(!strcmp(node_fs1,"\0"))
	fprintf(f, "%s\t%s\t%s\t%s\n",ind, tree->node_tkn, tree->node_tag,node_fs1);
//	else
//		printf("Guns don't need agreement");
	free(node_fs);
	free(node_fs1);
	int i, n=0;
	char ind_tmp[100],ind_append[10];
