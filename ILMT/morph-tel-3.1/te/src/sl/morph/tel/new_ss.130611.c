		...
		int* no_of_ans_left, int* no_of_ans_right)
{
	...
	{				// for every rule in rule pool
     ...
     //printf ("checking with 1-- %s\n", rule_pool->rule[i].pat);
		...
		{
			pattern_matched[i] = 0;
     //printf ("checking with 2-- %s\n", rule_pool->rule[i].pat);
//      printf ("Pattern Matched with %d\t", pattern_matched);
			break;
		}
		  //      printf("regexec\n");
		...
		{
			pattern_matched[i] = 0;
     //printf ("checking with 3-- %s\n", rule_pool->rule[i].pat);
		}
		else
			pattern_matched[i] = 1;
//     printf ("checking with 4-- %s\n", rule_pool->rule[i].pat);
//     printf ("left word tmp is-- %s\n", right_word_tmp);
     printf ("The right word temp is 1-- %s\n",right_word_tmp);
		...
		if (pattern_matched[i] == 1)
		{
			...
			if (split_pos[0].rm_so == 0)
			{
				...
				substr (input_word, split_pos[0].rm_eo, strlen (input_word),
						right_word_tmp);
     printf ("The right word temp is 2-- %s\n",right_word_tmp);
     //printf ("checking with 5-- %s\n", rule_pool->rule[i].pat);
			}
			else
			{
				substr (input_word, 0,
     ...
     //printf ("checking with 6-- %s\n", rule_pool->rule[i].pat);
     printf ("The right word temp is %s\n",right_word_tmp);
			}
