            ...
            for eachvalue in definitions[types[index]]:
                ...
                if creating == 1:
                    if eachvalue in problemList:
                        s += eachvalue + ' => ' + createTable(definitions, ['Tam', '_', 'Case', 'Gen', 'Num'], problemList, defvar, index+1, flag=semicolFlag)
                    elif types[index] == 'Tam':
                        s += eachvalue + ' => ' + createTable(definitions, ['Tam', 'Per', '_', 'Gen', 'Num'], problemList, defvar, index+1, flag=semicolFlag)
                    else:
                        s += eachvalue + ' => ' + createTable(definitions, types, problemList, defvar, index+1, flag=semicolFlag)
                elif creating == 2:
                    if types[index] == 'Tam':
                        if eachvalue == 'zero':
                            s += eachvalue + ''
